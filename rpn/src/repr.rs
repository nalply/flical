...
pub fn repr_z(z: Z) -> Repr {
  Repr::from_bytes(disp_z(z, Internal))
}

pub fn repr_r(r: R) -> Repr {
  Repr::from_bytes(disp_r(r, Internal))
}

pub fn repr_q(q: Q) -> Repr {
  Repr::from_bytes(disp_q(q, Internal))
}

pub fn repr_c(c: C) -> Repr {
  Repr::from_bytes(disp_c(c, Internal))
}

/// The repr is the internal representation of numbers. The calculator has a
/// defined display precision of 12 decimal digits and one more internal
/// precision digit. Integers are strings of 16 hex digits prefixed with `H`.
/// Reals are strings of 13 decimal digits followed by `E` and an integer in
/// `[-299..=299]` as a string, the thirteenth digit is hidden. Quotients are
/// two numbers in `[0..=999_999]` as string and separated by `/` and optionally
/// prefixed by `-`.
///
/// Examples `"Hdeafcafebabec001"`, `"H0000000000000000"`, `"Hfedcba9876543210"`,
/// `"1/2"`, "-123/13", "`"0000000000001E-1"`, `"0000000000042E-2"`,
/// `"1234567890123E299"` and `"0000000000000E0i0000000000001E0"`
///
...
/// the calculator numbers are not the native numbers. I could redefine the
/// type Z to a big integer type or R to a posit number later but now the
/// calculator specification is:
///
/// - Z: 64 bit signed integer
/// - R: floating point number with 12 decimal digits precision and exponent
/// between -300 and 300 exclusive, and also infinities +oo and -oo.
/// - Q: quotients with numerator and denominator not more than 6 decimal digits
/// - C: two values of R, one for the real and the other for the imaginary part
///
...
/// be represented as `"H0000000000000000"`.
#[derive(Clone, Copy, PartialEq, Eq)]
pub struct Repr([u8; REPR_LEN]);

pub const REPR_LEN: usize = 37;
...
impl Repr {
  pub const ZERO: Repr = Repr(*b"H0000000000000000                    ");
...
mod tests {
  ...
  fn test_disp_infinity() {
    ...
    assert_eq!(Native::r(f64::INFINITY).disp(Internal), "+oo");
    assert_eq!(Native::r(-f64::INFINITY).disp(Std), "-oo");
    assert_eq!(Native::r(-f64::INFINITY).disp(Fix(4)), "-oo");
    assert_eq!(Native::r(-f64::INFINITY).disp(Sci(4)), "-oo");
    assert_eq!(Native::r(f64::INFINITY).disp(HexU), "+oo");
